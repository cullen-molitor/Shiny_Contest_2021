...
{ # Density   ----
  ...
  { # RDFC    ----
    RDFC_Density <- 
      ...
      dplyr::ungroup() 
    # %>% 
      # readr::write_csv("Tidy_Data/RDFC_Count.csv")
  ...
  { # VFT   ----
    ...
    # %>%
      # readr::write_csv("Tidy_Data/VFT_Density.csv") 
  }
  ...
  { # All Density    ----
    ...
    Density_CSV <- Benthic_Density_CSV %>% 
      ...
      readr::write_csv("Tidy_Data/Density.csv")
  }
...
{ # RPC % Cover   ----
  RPC_Cover <- readr::read_csv(
    ...
    readr::write_csv("Tidy_Data/RPC_Cover.csv")
  ...
  #   readr::write_csv("Meta_Data/Site_Info.csv")
...
{ # Diversity   ----
  ...
  { # Diversity Complete   ----
    Diversity <- Diversity_Shannon_All  %>% 
      ...
      readr::write_csv("Tidy_Data/Diversity.csv")
  }
...
{ # Size Frequencies & Biomass   ----
  ...
  { # Benthic Sizes  ----
    Benthic_Sizes <- base::rbind(Kelp_Sizes, Gorgonian_Sizes, Invert_Sizes) %>%
      ...
      dplyr::ungroup() %>% 
      readr::write_csv("Tidy_Data/Benthic_Sizes.csv") %>% 
  ...
  { # Fish Sizes  ----
    Fish_Sizes <- 
      ...
      dplyr::mutate(Total_Count = length(Size), Mean_Size = mean(Size), Date = max(Date)) %>% 
      dplyr::ungroup() %>% 
      readr::write_csv("Tidy_Data/Fish_Sizes.csv")
  ...
  { # Biomass All   -----
    ...
    Mean_Biomass_CSV <- Fish_Mean_Biomass %>% 
      ...
      readr::write_csv("Tidy_Data/Biomass.csv")
...
{ # ARMs  ----
  ...
  { # Sizes   -----
    ARM_Sizes <- 
      ...
      readr::write_csv("Tidy_Data/ARMs.csv")
  }
  ...
  { # Par par ----
    ARM_par_Sizes <- 
      ...
      readr::write_csv("Tidy_Data/ARMs_par.csv")
  }
...
{ # Mixed Data (% Cover, Count, Biomass) for Random Forest Model   ----
  ...
  { # Mixed Data   -----
    ...
    Mixed_All <- RPC_Cover %>% 
      ...
      readr::write_csv("Tidy_Data/Mixed_Data_All.csv")
    ...
    Mixed_2005 <- Mixed_All %>% 
      ...
      readr::write_csv("Tidy_Data/Mixed_Data_2005.csv")
  }
...
{ # Random Forest Important Species    ----
  ...
  { # All Years Reserve Model   -----
    ...
    Mixed_Data_All <- readr::read_csv("Tidy_Data/Mixed_Data_All.csv") 
    ...
    saveRDS(RF_Reserve_Model_All_Years, "Models/RF_Reserve_Model_All.rds")
  ...
  { # 2005 Reserve Model   -----
    ...
    Mixed_Data_2005 <- readr::read_csv("Tidy_Data/Mixed_Data_2005.csv") 
  ...
  { # All Years Island Model   -----
    ...
    saveRDS(RF_Island_Model_All, "Models/RF_Island_Model_All.rds")
    ...
    # %>% 
      # readr::write_csv("Tidy_Data/Species_Importance_All.csv")
  }
  ...
  { # 2005 Island Model   -----
    ...
    # %>% 
    # readr::write_csv("Tidy_Data/Species_Importance_2005.csv")
    
  }
  
  { # RF All ranked variables   ----
    RF_Importance <- base::rbind(RF_VI_Isl_All, RF_VI_Isl_2005) %>% 
      readr::write_csv("Tidy_Data/RF_Importance.csv")
  }
...
{ # Mixed Data nMDS Dimensions    ----
  ...
  { # nMDS All Output    ----
    ...
    readr::write_csv("Tidy_Data/nMDS.csv")
...
{ # GIS Data   ----
  ...
  CINP <- sf::st_read("Raw_Data/Shapefiles/california_islands.shp") %>%
    ...
    sf::st_write(dsn = "GIS_Data/CINP_Islands.gpkg", layer = "CINP_Islands", 
  ...
  mpa <- sf::st_read("Raw_Data/Shapefiles/California_Marine_Protected_Areas.shp") %>% 
    ...
    sf::st_write(dsn = "GIS_Data/CA_MPA.gpkg", layer = "CA_MPA", 
                 delete_dsn = TRUE, layer_options = "OVERWRITE=YES")
  
  GPS_Transects <- sf::st_read("Raw_Data/Shapefiles/KFM_Transects_SmoothLine5.shp")  %>%
    sf::st_as_sf() %>%
    dplyr::mutate(geometry = st_transform(geometry, "+proj=longlat +ellps=WGS84 +datum=WGS84")) %>%
    sf::st_write(dsn = "GIS_Data/KFM_Transects.gpkg", layer = "KFM_Transects", 
                 delete_dsn = TRUE, layer_options = "OVERWRITE=YES")
  
  NPS_boundary <- sf::st_read("Raw_Data/Shapefiles/nps_boundary.shp") %>%
    sf::st_as_sf() %>%
    dplyr::mutate(geometry = st_transform(geometry, "+proj=longlat +ellps=WGS84 +datum=WGS84")) %>%
    sf::st_write(dsn = "GIS_Data/CINP_Boundary.gpkg", layer = "CINP_Boundary", 
                 delete_dsn = TRUE, layer_options = "OVERWRITE=YES")
  
  CINMS_boundary <- sf::st_read("Raw_Data/Shapefiles/cinms_py.shp") %>%
    sf::st_as_sf() %>%
    dplyr::mutate(geometry = st_transform(geometry, "+proj=longlat +ellps=WGS84 +datum=WGS84")) %>%
    sf::st_write(dsn = "GIS_Data/CINMS_Boundary.gpkg", layer = "CINMS_Boundary", 
                 delete_dsn = TRUE, layer_options = "OVERWRITE=YES")
}
...
{ # Temperature RAW to Tidy   ----
  ...
  #   readr::write_csv("Tidy_Data/Temp_Raw_Tidy.csv")
...
{ # RATIOS - BEWARE... THESE TAKE A LOOOOONG TIME  -----------------
  ...
  { # Biomass Ratios   ----
    ...
    { # Data  ----
      Biomass_Data <- readr::read_csv("Tidy_Data/Biomass.csv") %>%
        dplyr::filter(
          # SiteCode != "KH",
          !ScientificName %in% c(
  ...
  { # All Ratios   -----
    All_Ratios <- 
      base::rbind(
        ...
        RDFC_Density_Ratios, 
        VFT_Density_Ratios) %>% 
      readr::write_csv("Tidy_Data/Ratios.csv")
  }
